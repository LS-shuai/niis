...
public class SimpleMailSender {
     ...
     * @throws MessagingException
     */
    public void send(List<String> recipients,List<String> recipients_Cc, String subject, Object content)
        throws AddressException, MessagingException {
    // 创建mime类型邮件
    final MimeMessage message = new MimeMessage(session);
    // 设置发信人
    ...
    message.setRecipients(RecipientType.TO, addresses);
    //设置抄送人们
    if(recipients_Cc.size()>0)
    {
	    final int num_Cc = recipients_Cc.size();
	    InternetAddress[] addresses_Cc = new InternetAddress[num_Cc];
	    for (int i = 0; i < num_Cc; i++) {
	    	addresses_Cc[i] = new InternetAddress(recipients_Cc.get(i));
	    }
	    message.setRecipients(RecipientType.CC, addresses_Cc);
    }
    // 设置主题
    message.setSubject(subject);
    // 设置邮件内容
    ...
    public static void main(String[] args) {
		...
		try {
			sender.send(receiveNames,receiveNames, "测试", "hello");
